  ...
  try {
    ...
    if (result?.composerReady) {
      if (job.workerWasActivated) {
        await chrome.tabs.update(job.ownerTabId, { active: true }).catch(() => {});
...
async function finishJob(tabId, job) {
  ...
  setTimeout(() => chrome.tabs.remove(tabId).catch(() => {}), 2200);
}
...
async function loadNextAttempt(workerTabId, job) {
  ...
  job.dispatched = false;
  await chrome.tabs.update(workerTabId, { url: job.url });
...
chrome.runtime.onMessage.addListener(async (message, sender) => {
  ...
  if (message?.type === "tmr-get-batch-status") {
    const stored = await chrome.storage.local.get("tmrLastBatch");
